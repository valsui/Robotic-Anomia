...
model.add(tf.layers.dense({
    ...
    kernelInitializer: 'VarianceScaling',
    activation: 'softmax' //activation function of the last layer fro the classification task - usuall softmax which normalizes output vector into a probability distribution
}));
...
for (let i = 0; i < TRAIN_BATCHES; i++) {
    ...
    let testBatch;
    let validationData;
    //test the batches for accuracy
    if( i % TEST_ITERATION_FREQUENCY === 0 ){
        testBatch = dta.nextTestBatch(TEST_BATCH_SIZE);
        validationData = [
            testBatch.data.reshape([TEST_BATCH_SIZE, 25, 25, 1]), testBatch.output
        ];
    }
